...
class AchievementSupport extends Component {
  ...
  handleSubmit = () => {
    const { onSupport, recipientWalletAddress } = this.props
    const { amount, fees } = this.state
    onSupport({ amount, fees: FeesSelector.convertFees(fees), recipientWalletAddress })
    this.resetForm()
    this.handleClose()
  }
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            margin="normal"
            id='amount'
            label={`Amount in DCT - maximum ${walletBalance} DCT minus fees)`}
            value={amount}
...
AchievementSupport.propTypes = {
  ...
  recipientWalletAddress: T.number
}
