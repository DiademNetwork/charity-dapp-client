...
class Achievement extends Component {
  ...
  handleConfirm = () => {
    const { accessToken, confirmAchievement, userID, walletAddress, facebookName } = this.props
    const { displayedHistoryItem: { object, wallet: recipientWalletAddress } } = this.state
    confirmAchievement({
  ...
  render () {
    ...
    if (!displayedHistoryItem) return null

    const { actor: creatorID, name: creatorName, title, object, wallet: recipientWalletAddress } = displayedHistoryItem
    ...
    return [
      <Card key="achievement-card" className={classes.card}>
        ...
        <CardContent>
          ...
          {supportsCount > 0 &&
            <Typography variant="body1" color="textSecondary">
              It has been supported by {R.head(uniqSupportersNames)}{supportsCount - 1 > 0 ? ` and ${supportsCount - 1} others` : ''} for a total amount of {supportsTotalAmount} DCT
            </Typography>
          }
          {despositsCount > 0 &&
            <Typography variant="body1" color="textSecondary">
              {R.head(uniqDepositorsNames)}{despositsCount - 1 > 0 ? ` and ${despositsCount - 1} others have` : ' has'} made a deposit for a total amount of {depositsTotalAmount} DCT
            </Typography>
          }
        </CardContent>
        {creatorID !== userID ? (
